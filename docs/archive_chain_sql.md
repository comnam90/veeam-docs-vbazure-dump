...
1. During the first archive session, Veeam Backup for Microsoft Azure detects backed-up data that is stored in the full backup and all incremental backups existing in the [backup chain](backup_chain_sql.md), creates a full archive backup with all the data, and copies this backup to the archive repository. The full archive backup becomes a starting point in the archive chain.
